...
class Burpa:
    ...
    def __init__(self, api_url: str = "http://127.0.0.1",
                api_port: str = "8090",
                new_api_url: str = "http://127.0.0.1",
    ...
    def _wait_scan(self, *records: ScanRecord, timeout: int = None) -> None:
        """
        ...
        # Calculate the end time if timeout is specified
        ...
        while not statuses or any(status not in SCAN_STATUS_FINISHED for status in statuses):
            ...
            if end_time and current_time >= end_time:
                self._logger.info(f"Timeout reached. Stopping scan(s).")
                self.stopped=True
                break
    ...
    def scan(self, *targets: str, report_type: str = "HTML", 
             ...
             issue_confidence:Union[str, Tuple[str, ...]]="All", csv:bool=False,  timeout: int = None) -> List[ScanRecord]:
        """
        ...
        self._wait_scan(*records, timeout=timeout)
        ...
        if self.stopped:
            ...
            stop_thread = threading.Thread(target=self._stop(), args=(delay,))
            stop_thread.start()
        return records
